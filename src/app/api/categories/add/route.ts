...
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(
            `${API_BASE_URL}/ProductApi/ICategoryFeature/AddCategory`,
            {
                method: 'POST',
                headers: {
        ...
        if (!response.ok) {
            ...
            console.error(`[AddCategory] Backend error (${response.status}):`, errorText);
            return NextResponse.json(
                {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            console.error('[AddCategory] Expected JSON but got:', text);
            return NextResponse.json(
        ...
        console.error('[AddCategory] SSR error:', error);
        return NextResponse.json(
