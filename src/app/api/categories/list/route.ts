...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`[ListCategories] Backend error (${response.status}):`, errorText);
            return NextResponse.json(
                {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            console.error('[ListCategories] Expected JSON but got:', text);
            return NextResponse.json(
        ...
        console.error('[ListCategories] SSR error:', error);
        return NextResponse.json(
