...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[AddProduct] Backend error (${response.status}):`, errorText);
      return NextResponse.json(
        {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      console.error('[AddProduct] Expected JSON but got:', text);
      return NextResponse.json(
    ...
    console.error('[AddProduct] SSR error:', error);
    return NextResponse.json(
